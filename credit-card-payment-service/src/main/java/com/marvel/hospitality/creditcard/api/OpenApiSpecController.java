...
/**
 * Serves the hand-corrected {@code docs/contracts/credit-card-payment-api.yaml} verbatim at
 * {@code /v3/api-docs} instead of springdoc's own introspected document (build.gradle copies the file into
 * the jar's {@code static/} at build time; it is never committed here). springdoc's generated resource at
...
class OpenApiSpecController {
    ...
    OpenApiSpecController(@Value("classpath:static/credit-card-payment-api.yaml") Resource specResource) {
